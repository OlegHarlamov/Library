...
function createUser($connection,$username,$password, $first_name, $last_name)
{
    ...
    mysqli_stmt_bind_param($stmt,"ssss",$username,$pwdhashed, $first_name, $last_name);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    header("Location: ../register.php?registration=success");
}

function addBook($connection, $title, $text, $author, $uploadedfile)
{
    $sql = "INSERT INTO books (title, content, author, image) VALUES (?,?,?,?);";
    $stmt = mysqli_stmt_init($connection);
    if (!mysqli_stmt_prepare($stmt,$sql))
    {
        header("Location: ../addBooks.php?error=stmt");
        exit();
    }
    mysqli_stmt_bind_param($stmt,"ssss",$title, $text, $author, $uploadedfile);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    header("Location: ../addBooks.php?addeding=success");
}
...
function userLogin($connection,$username,$password)
{
    ...
    else if ($checkpwd == true)
    {
        ...
        $_SESSION['permission'] = $userExists['permission'];
